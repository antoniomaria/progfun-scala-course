...
class TweetSetSuite extends munit.FunSuite:
  trait TestSets:
    ...
    val set6 = set2.incl(c).incl(d).incl(e)
  ...
  test("mostRetweeted: with set (6)") {
    new TestSets {
      assertEquals(set6.mostRetweeted.retweets, 22)
    }
  }

  test("mostRetweeted: with set (3)") {
    new TestSets {
      val mostRetweeted = set3.mostRetweeted
      assertEquals(mostRetweeted.retweets , 20)
      assertEquals(mostRetweeted.user , "b")
    }
  }
