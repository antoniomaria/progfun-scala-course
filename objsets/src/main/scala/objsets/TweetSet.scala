...
abstract class TweetSet extends TweetSetInterface:
   ...
   * Question: Should we implement this method here, or should it remain abstract
   * and be implemented in the subclasses?
   * At least for NonEmptySet should be implemented in subclass
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet ={
    //((left union right) union that) incl elem //we learn this in lecture 3.1 (INEFFICIENT)
  ...
  override def mostRetweeted : Tweet = {
    def mostRetweeted(tweet1: Tweet, tweet2: Tweet): Tweet = 
      if (tweet1.retweets > tweet2.retweets) then tweet1 else tweet2
    
    if right.isEmpty & left.isEmpty then
      elem
    else if !right.isEmpty && left.isEmpty then
      mostRetweeted(tweet1 = elem, tweet2 = right.mostRetweeted)
    else if right.isEmpty && !left.isEmpty then
      mostRetweeted(tweet1 = elem, tweet2 = left.mostRetweeted)
    else 
      mostRetweeted(tweet1 = mostRetweeted(right.mostRetweeted, elem), tweet2 = left.mostRetweeted)
  }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet =>{
    google.find(x=>(tweet.text.contains(x))).isDefined
  })

  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(
      tweet=>{
        apple.exists( s => tweet.text.contains(s))
      }
    )
  }
