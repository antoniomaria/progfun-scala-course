...
class AnagramsSuite extends munit.FunSuite:
  ...
  test("wordOccurrences: abcd (3pts)") {
    assertEquals(wordOccurrences("abcdd"), List(('a', 1), ('b', 1), ('c', 1), ('d', 2)))
  }
  ...
  test("sentenceOccurrences: Roberto Carlos e (5pts)") {
    println (sentenceOccurrences(List("Roberto", "Carlos")) )
  }


  test("dictionaryByOccurrences.get: eat (10pts)") {
    val teaOcurrency = dictionaryByOccurrences.get(List(('a', 1), ('e', 1), ('t', 1)))
    println ("tea ocurrency is  " +teaOcurrency)
  ...
  test("combinations: abbacc (8pts)") {
    val abba = List(('a', 2), ('b', 2))
    combinations(abba).foreach(println (_))
    println ("****** ")
    combinations(abba).foreach(println (_))
    println ("### ")
    val abbacomb = List(
